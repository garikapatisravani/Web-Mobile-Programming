...
public class MainActivity extends AppCompatActivity {
    ...
    final int CHEDDARCHESE_PRICE = 1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cheddarchese = findViewById(R.id.cheddarcheese);
        mushrooms = findViewById(R.id.mushrooms);
        tomatos =  findViewById(R.id.tomatos);
    ...
    public void sendEmail() {
        ...
        boolean hasCheddarChese = cheddarchese.isChecked();

        boolean hasTomatos = tomatos.isChecked();

        boolean hasMushrooms = mushrooms.isChecked();

        float totalPrice = calculatePrice(hasChicken, hasCheddarChese, hasTomatos, hasMushrooms);

        // create and store the order summary
        String orderSummaryMessage = createOrderSummary(userInputName, hasChicken, hasCheddarChese, hasTomatos, hasMushrooms, totalPrice);
    ...
    private String createOrderSummary(String userInputName, boolean hasChicken, boolean hasCheddarChese, boolean hasTomatos, boolean hasMushrooms, float price) {
        String orderSummaryMessage = getString(R.string.order_summary_name, userInputName) + "\n\n" +
                getString(R.string.order_details) + "\n" +
                getString(R.string.order_summary_topping1, boolToString(hasChicken)) + "\n" +
                getString(R.string.order_summary_topping2, boolToString(hasCheddarChese)) + "\n" +
     ...
     * @return total Price
     */
    private float calculatePrice(boolean hasChicken, boolean hasCheddarChese, boolean hasTomatos, boolean hasMushrooms) {
        int basePrice = PIZZA_PRICE;
        if (hasChicken) {
            basePrice += CHICKEN_PRICE;
        }
        if (hasCheddarChese) {
            basePrice += CHEDDARCHESE_PRICE;
        }
        if (hasTomatos) {
